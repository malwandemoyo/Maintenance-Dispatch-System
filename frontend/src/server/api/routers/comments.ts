  ...
  list: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        console.debug(
          `[tRPC/comments.list] forwarding cookie present=${!!cookie} value="${cookie.substring(0, 40)}..."`,
        );
        ...
        if (!response.ok) {
          ...
          console.debug(
            `[tRPC/comments.list] backend responded status=${response.status} body="${body}"`,
          );
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        console.debug(
          `[tRPC/comments.create] forwarding cookie present=${!!cookie} csrfToken=${!!csrfToken}`,
        );
        const headers: Record<string, string> = {
          "Content-Type": "application/json",
        };
        ...
        if (!response.ok) {
          ...
          console.debug(
            `[tRPC/comments.create] backend responded status=${response.status} body="${errorText}"`,
          );
          try {
        ...
        const data = await response.json();
        console.debug(
          `[tRPC/comments.create] backend response OK, data:`,
          data,
        );
        return data;
      } catch (error) {
