...
export default function ReportActions({ id }: { id: string | number }) {
  ...
  async function closeReport() {
    setLoading(true)
    try {
      const res = await fetch(`/api/backend/api/reports/${id}/close/`, { method: 'POST' })
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
  ...
  return (
    <div style={{ marginTop: 20 }}>
      <button onClick={createTask} disabled={loading} style={{ marginRight: 8 }}>
        Create Task
      </button>
      <button onClick={closeReport} disabled={loading}>Close Report</button>
    </div>
  )
}
